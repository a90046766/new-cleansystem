...
export async function computeMonthlyPayroll(month) {
    ...
    for (const t of technicians) {
        if (t.email)
            emailToTech[t.email.toLowerCase()] = t;
        nameToTech[t.name] = t;
        codeToTech[t.code?.toUpperCase?.() || ''] = t;
    }
    const perTechService = {};
    ...
    for (const t of technicians) {
        ...
        if (scheme.startsWith('pure')) {
            ...
            const rateMap = { base1: 0.1, base2: 0.2, base3: 0.3 };
...
export function getPayoutDates(month) {
